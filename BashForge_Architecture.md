    ...
    steps:
      ...
      - name: Build & push sandbox
        run: |
          ...
          docker push $IMAGE
          # Also tag as :latest so minikube pulls it
          docker tag $IMAGE ${{ steps.login-ecr.outputs.registry }}/bashforge-sandbox:latest
          docker push ${{ steps.login-ecr.outputs.registry }}/bashforge-sandbox:latest
...
| Secret                  | Value                               |
| ----------------------- | ----------------------------------- |
| `AWS_ACCESS_KEY_ID`     | IAM user key (ECR push + pull only) |
...
#   - redis
#   - A mock "pod" container (just the sandbox image, always running)
#     so you can develop the IDE without spinning K8s
```

The mock pod mode: in development, `k8s_client.py` reads an env var `MOCK_K8S=true` and instead of calling the K8s API, it just returns `localhost:8765` as the WebSocket target (pointing to the always-on sandbox container in docker-compose). This lets you develop and test the entire IDE UI without any Kubernetes.
